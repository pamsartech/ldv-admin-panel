    ...
    const fetchPayment = async () => {
      try {
        const response = await axios.get(
          `https://la-dolce-vita.onrender.com/api/payment/payment-details/${paymentId}`
        );
...
const handleDelete = async () => {
  ...
  try {
    ...
    const response = await axios.delete(
      `https://la-dolce-vita.onrender.com/api/payment/delete-payment/${paymentId}`
    );

    if (response.data.success) {
      alert("Payment deleted successfully!");
      navigate("/Payments"); // Redirect after delete
    } else {
    ...
    <div>
      ...
      <div className="flex justify-between mt-5 mx-10">
        ...
        <div>
          ...
          <button
           onClick={() => navigate(`/update-payment/${paymentData._id}`, { state: { paymentData } })  }
