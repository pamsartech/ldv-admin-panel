...
export default function PaymentDataTable() {
  ...
  const filteredPayments =
    filterMethod === "all" ? tabFiltered : tabFiltered.filter((p) => p.method === filterMethod);
...
<div className="flex justify-between items-center border-2 mb-5 border-gray-300 px-6  rounded-md p-2 relative">
  ...
  <div className="flex gap-6">
    ...
    <button
      onClick={() => { setActiveTab("Refunded"); setCurrentPage(1); }}
      className={`flex items-center gap-2 text-sm px-2 pb-1 ${
        activeTab === "Refunded"
          ? "text-black font-medium border-b-2 border-black"
          : "text-gray-600 hover:text-black"
    ...
    {showMethodFilter && (
      <div className="absolute right-0 top-12 w-48 bg-white border rounded-md shadow-md p-2 z-50">
        {["all", "Stripe", "PayPal"].map((method) => (
          <button
            key={method}
            onClick={() => { setFilterMethod(method); setCurrentPage(1); setShowMethodFilter(false); }}
                ...
                <tr key={p.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-3">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${statusStyles[p.status]}`}>
                      <FontAwesomeIcon icon={statusIcons[p.status]} /> {p.status}
                    </span>
                  </td>
